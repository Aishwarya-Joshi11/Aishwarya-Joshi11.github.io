...
SQL_STRINGS = {
    ...
    "insert_iris": {
        "sqlite": """INSERT INTO AQI VALUES(?, ?, ?, ?, ?)""",
        "mysql": """INSERT INTO AQI VALUES(%s, %s, %s, %s, "%s");""",
        "postgresql": """INSERT INTO AQI VALUES(%s, %s, %s, %s, %s);""",
    },
    "create_test_types": {
    ...
    "read_parameters": {
        "sqlite": "SELECT * FROM AQI WHERE Name=? AND SepalLength=?",
        "mysql": 'SELECT * FROM AQI WHERE `Name`="%s" AND `SepalLength`=%s',
        "postgresql": 'SELECT * FROM AQI WHERE "Name"=%s AND "SepalLength"=%s',
    },
    "read_named_parameters": {
        "sqlite": """
                SELECT * FROM AQI WHERE Name=:name AND SepalLength=:length
                """,
        "mysql": """
    ...
    "read_no_parameters_with_percent": {
        "sqlite": "SELECT * FROM AQI WHERE Name LIKE '%'",
        "mysql": "SELECT * FROM AQI WHERE `Name` LIKE '%'",
        "postgresql": "SELECT * FROM AQI WHERE \"Name\" LIKE '%'",
    },
    "create_view": {
        "sqlite": """
...
class PandasSQLTest:
    ...
    @pytest.fixture(params=[("io", "data", "csv", "AQI.csv")])
    ...
    def _read_sql_iris(self):
        iris_frame = self.pandasSQL.read_query("SELECT * FROM AQI")
    ...
    def _execute_sql(self):
        ...
        iris_results = self.pandasSQL.execute("SELECT * FROM AQI")
...
class _TestSQLApi(PandasSQLTest):
    ...
    def test_read_sql_iris(self):
        iris_frame = sql.read_sql_query("SELECT * FROM AQI", self.conn)
        self._check_iris_loaded_frame(iris_frame)
    ...
    def test_execute_sql(self):
        # drop_sql = "DROP TABLE IF EXISTS test"  # should already be done
        iris_results = sql.execute("SELECT * FROM AQI", con=self.conn)
        row = iris_results.fetchone()
        tm.equalContents(row, [5.1, 3.5, 1.4, 0.2, "Iris-setosa"])
...
class TestSQLApi(SQLAlchemyMixIn, _TestSQLApi):
    ...
    def test_read_sql_delegate(self):
        iris_frame1 = sql.read_sql_query("SELECT * FROM AQI", self.conn)
        iris_frame2 = sql.read_sql("SELECT * FROM AQI", self.conn)
        tm.assert_frame_equal(iris_frame1, iris_frame2)

        iris_frame1 = sql.read_sql_table("AQI", self.conn)
        iris_frame2 = sql.read_sql("AQI", self.conn)
        tm.assert_frame_equal(iris_frame1, iris_frame2)
    ...
    def test_query_by_text_obj(self):
        # WIP : GH10846
        name_text = sqlalchemy.text("select * from AQI where name=:name")
...
class TestSQLiteFallbackApi(SQLiteMixIn, _TestSQLApi):
    ...
    def test_con_string_import_error(self):
        ...
        with pytest.raises(ImportError, match=msg):
            sql.read_sql("SELECT * FROM AQI", conn)
    ...
    def test_read_sql_delegate(self):
        ...
        msg = "Execution failed on sql 'AQI': near \"AQI\": syntax error"
        with pytest.raises(sql.DatabaseError, match=msg):
            sql.read_sql("AQI", self.conn)
...
class _TestSQLAlchemy(SQLAlchemyMixIn, PandasSQLTest):
    ...
    def test_read_table(self):
        iris_frame = sql.read_sql_table("AQI", con=self.conn)
        self._check_iris_loaded_frame(iris_frame)

    def test_read_table_columns(self):
        iris_frame = sql.read_sql_table(
            "AQI", con=self.conn, columns=["SepalLength", "SepalLength"]
        )
